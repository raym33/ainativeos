export type Lang = "es" | "en";

const STORAGE_KEY = "rworkbench.lang";

export function detectLang(): Lang {
  if (typeof navigator !== "undefined" && navigator.language.toLowerCase().startsWith("es")) {
    return "es";
  }
  return "en";
}

export function loadLang(): Lang {
  try {
    if (typeof localStorage === "undefined") {
      return detectLang();
    }
    const stored = localStorage.getItem(STORAGE_KEY);
    return stored === "es" || stored === "en" ? stored : detectLang();
  } catch {
    return detectLang();
  }
}

export function saveLang(lang: Lang): void {
  try {
    if (typeof localStorage !== "undefined") {
      localStorage.setItem(STORAGE_KEY, lang);
    }
  } catch {
    // Ignore storage failures in private browsing, SSR, or locked-down contexts.
  }
}

export const translations: Record<Lang, Record<string, string>> = {
  en: {
    "welcome.greeting": "What can I help you with?",
    "welcome.sub": "Ask in your own words. I'll tell you if I can do it and what I need — and I always ask before doing anything important.",
    "intent.title": "What do you want to do?",
    "intent.placeholder": "Describe it in your own words, e.g. \"summarize these PDFs\" or \"draft an email to a client\"",
    "intent.check": "Check",
    "intent.examplesLabel": "Examples:",
    "intent.verdict.ready": "Yes, I can do this now.",
    "intent.verdict.setup": "I can do this once a few things are set up.",
    "intent.verdict.unknown": "I'm not sure I can do that directly.",
    "intent.how": "How it works",
    "intent.youProvide": "What I need from you",
    "intent.setupNeeded": "Set up first",
    "intent.guardrail": "Guardrail",
    "intent.guardrail.general": "Nothing runs without your go-ahead, and I keep your original files.",
    "intent.guardrail.approval": "Before anything leaves your computer I show you exactly what will be done and wait for your Approve or Cancel.",
    "intent.ask": "Ask the assistant to do it",
    "intent.unknownHelp": "You can still ask the assistant — it will look for a way and tell you what it finds.",
    "intent.diagnostics": "Check what's ready",
    "approval.aria": "Action approval",
    "approval.approve": "Approve",
    "approval.cancel": "Cancel",
    "approval.note": "Nothing runs until you choose.",
    "approval.title": "Approve before continuing",
    "aria.chat": "Chat",
    "aria.coreFunctions": "Core functions",
    "aria.langToggle": "Language",
    "aria.message": "Message",
    "aria.permissionSummary": "Permission summary",
    "aria.systemCapabilities": "System capabilities",
    "aria.systemStatus": "System status",
    "aria.toolSummary": "Tool summary",
    "brand.eyebrow": "Local workspace",
    "brand.subtitle": "Private automation console",
    "composer.pending": "Approve or cancel the pending action above to continue...",
    "composer.placeholder": "Type what you need...",
    "composer.send": "Send",
    "composer.sendTooltip": "Send this task to the local agent.",
    "composer.stop": "Stop",
    "composer.stopTooltip": "Stop the current response.",
    "collections.empty": "No saved research notes yet. Run Web research, then use Save note.",
    "collections.eyebrow": "Research library",
    "collections.exportBoth": "Export both",
    "collections.exportPdf": "Export PDF",
    "collections.exportText": "Export text",
    "collections.refreshTooltip": "Refresh saved research notes from the workspace.",
    "collections.rootPending": "Workspace will be created when the first note is saved.",
    "collections.select": "Select a note to preview it here.",
    "collections.title": "Research collections",
    "dashboard.copy":
      "Eve routes requests to LM Studio and the local R skill catalog. The interface keeps the model focused, exposes tool activity, and blocks sensitive actions by default.",
    "dashboard.eyebrow": "Desktop-grade local AI",
    "dashboard.heading": "Start with a task, not a prompt.",
    "forge.heading": "Skill Forge",
    "health.checking": "Checking",
    "health.detail.bridgeMissing": "Run `npm run r:install`",
    "health.detail.bridgeReady": "Python bridge files are present",
    "health.detail.catalogMissing": "Run `npm run r:catalog`",
    "health.detail.eveOffline": "Not reachable — run `npm run start:local`",
    "health.detail.healthUnavailable": "Start the Vite web server to enable local diagnostics.",
    "health.detail.lexiaOptional": "Optional legal RAG is not running",
    "health.detail.lmModelMissing": "Model not found in LM Studio — set LM_STUDIO_MODEL",
    "health.detail.lmOffline": "Can't reach LM Studio — start its local server",
    "health.detail.readerDirect": "Direct page extraction only",
    "health.detail.searchFallback": "DuckDuckGo fallback only",
    "health.eyebrow": "Setup status",
    "health.label.health": "Health",
    "health.label.evebackend": "Eve backend",
    "health.label.lexia": "Lexia",
    "health.label.lmstudio": "LM Studio",
    "health.label.pagereader": "Page reader",
    "health.label.rcatalog": "R catalog",
    "health.label.rbridge": "R bridge",
    "health.label.websearch": "Web search",
    "health.label.workspace": "Workspace",
    "health.loadingDetail": "Running local checks.",
    "health.loadingLabel": "Diagnostics",
    "health.needsSetup": "needs setup",
    "health.ready": "ready",
    "health.refresh": "Refresh",
    "health.refreshTooltip": "Run the local setup checks again.",
    "health.state.info": "info",
    "health.state.offline": "offline",
    "health.state.ready": "ready",
    "health.state.warning": "check",
    "health.title": "System readiness",
    "model.fallback": "LM Studio local",
    "model.label": "Model",
    "permissions.blocked": "blocked",
    "permissions.blockedSkills": "Blocked skills",
    "permissions.copy": "Sensitive skills are blocked by default. To unlock them, start Eve with",
    "permissions.loading": "Loading local permission profile.",
    "permissions.ready": "ready",
    "permissions.title": "Permissions",
    "quickActions.aria": "Quick actions",
    "research.copy": "Search, read, rank, and answer from cited sources.",
    "research.title": "Web research",
    "reset.label": "New session",
    "reset.tooltip": "Clear the current Eve session and start fresh.",
    "skills.catalogMissing": "Run `npm run r:catalog` if the catalog does not appear.",
    "skills.fallbackDescription": "Local R skill.",
    "skills.loading": "loading",
    "skills.searchAria": "Search R tools",
    "skills.searchPlaceholder": "Search PDF, CSV, QR...",
    "skills.title": "R skills",
    "skills.use": "Use",
    "skills.useTooltip": "Create a guided prompt for this exact R tool.",
    "status.backend": "Eve backend through local proxy",
    "status.error": "Error",
    "status.ready": "Ready",
    "status.responding": "Responding",
    "status.sending": "Sending",
    "system.blockedByDefault": "blocked by default",
    "system.skillsIndexed": "skills indexed",
    "system.toolsAvailable": "tools available",
    "tools.label": "Tools",
    "tools.value": "R skills / Web research / Firecrawl",
    "toolLog.empty": "No runs yet",
    "toolLog.finished": "finished",
    "toolLog.history": "History",
    "toolLog.muted": "Tool activity will appear here when the agent acts.",
    "toolLog.running": "running",
    "toolLog.runsThisSession": "runs this session",
    "toolLog.tools": "tools",
    "user.you": "You",
    "workbench.pdf.copy": "OCR, summarize, merge, extract, repair, and generate reports locally.",
    "workbench.pdf.title": "PDF workbench",
  },
  es: {
    "welcome.greeting": "¿En qué te ayudo?",
    "welcome.sub": "Pídemelo con tus palabras. Te diré si puedo hacerlo y qué necesito, y siempre te pregunto antes de hacer algo importante.",
    "intent.title": "¿Qué quieres hacer?",
    "intent.placeholder": "Descríbelo con tus palabras, p. ej. \"resume estos PDFs\" o \"redacta un email a un cliente\"",
    "intent.check": "Comprobar",
    "intent.examplesLabel": "Ejemplos:",
    "intent.verdict.ready": "Sí, puedo hacerlo ahora.",
    "intent.verdict.setup": "Puedo hacerlo en cuanto prepares un par de cosas.",
    "intent.verdict.unknown": "No estoy seguro de poder hacer eso directamente.",
    "intent.how": "Cómo funciona",
    "intent.youProvide": "Qué necesito de ti",
    "intent.setupNeeded": "Prepara antes",
    "intent.guardrail": "Guardarraíl",
    "intent.guardrail.general": "Nada se ejecuta sin tu visto bueno, y conservo tus archivos originales.",
    "intent.guardrail.approval": "Antes de que algo salga de tu ordenador te muestro exactamente qué se hará y espero tu Aprobar o Cancelar.",
    "intent.ask": "Pídeselo al asistente",
    "intent.unknownHelp": "Aun así puedes pedírselo al asistente: buscará la manera y te dirá qué encuentra.",
    "intent.diagnostics": "Ver qué está listo",
    "approval.aria": "Aprobacion de accion",
    "approval.approve": "Aprobar",
    "approval.cancel": "Cancelar",
    "approval.note": "Nada se ejecuta hasta que elijas.",
    "approval.title": "Aprueba antes de continuar",
    "aria.chat": "Chat",
    "aria.coreFunctions": "Funciones principales",
    "aria.langToggle": "Idioma",
    "aria.message": "Mensaje",
    "aria.permissionSummary": "Resumen de permisos",
    "aria.systemCapabilities": "Capacidades del sistema",
    "aria.systemStatus": "Estado del sistema",
    "aria.toolSummary": "Resumen de herramientas",
    "brand.eyebrow": "Espacio de trabajo local",
    "brand.subtitle": "Consola privada de automatizacion",
    "composer.pending": "Aprueba o cancela la accion pendiente de arriba para continuar...",
    "composer.placeholder": "Escribe lo que necesitas...",
    "composer.send": "Enviar",
    "composer.sendTooltip": "Enviar esta tarea al agente local.",
    "composer.stop": "Detener",
    "composer.stopTooltip": "Detener la respuesta actual.",
    "collections.empty": "Aun no hay notas de investigacion guardadas. Ejecuta Investigacion web y luego Guardar nota.",
    "collections.eyebrow": "Biblioteca de investigacion",
    "collections.exportBoth": "Exportar ambos",
    "collections.exportPdf": "Exportar PDF",
    "collections.exportText": "Exportar texto",
    "collections.refreshTooltip": "Actualizar las notas guardadas desde el workspace.",
    "collections.rootPending": "El workspace se creara al guardar la primera nota.",
    "collections.select": "Selecciona una nota para previsualizarla aqui.",
    "collections.title": "Colecciones de investigacion",
    "dashboard.copy":
      "Eve dirige las solicitudes a LM Studio y al catalogo local de habilidades R. La interfaz mantiene el modelo enfocado, muestra la actividad de las herramientas y bloquea las acciones sensibles por defecto.",
    "dashboard.eyebrow": "IA local de escritorio",
    "dashboard.heading": "Empieza con una tarea, no con un prompt.",
    "forge.heading": "Forja de habilidades",
    "health.checking": "Comprobando",
    "health.detail.bridgeMissing": "Ejecuta `npm run r:install`",
    "health.detail.bridgeReady": "Los archivos del puente Python estan presentes",
    "health.detail.catalogMissing": "Ejecuta `npm run r:catalog`",
    "health.detail.eveOffline": "No accesible — ejecuta `npm run start:local`",
    "health.detail.healthUnavailable": "Arranca el servidor web de Vite para activar el diagnostico local.",
    "health.detail.lexiaOptional": "El RAG legal opcional no esta arrancado",
    "health.detail.lmModelMissing": "Modelo no encontrado en LM Studio — define LM_STUDIO_MODEL",
    "health.detail.lmOffline": "No se puede conectar con LM Studio — arranca su servidor local",
    "health.detail.readerDirect": "Solo extraccion directa de paginas",
    "health.detail.searchFallback": "Solo fallback de DuckDuckGo",
    "health.eyebrow": "Estado de setup",
    "health.label.health": "Salud",
    "health.label.evebackend": "Backend Eve",
    "health.label.lexia": "Lexia",
    "health.label.lmstudio": "LM Studio",
    "health.label.pagereader": "Lector web",
    "health.label.rcatalog": "Catalogo R",
    "health.label.rbridge": "Puente R",
    "health.label.websearch": "Busqueda web",
    "health.label.workspace": "Workspace",
    "health.loadingDetail": "Ejecutando comprobaciones locales.",
    "health.loadingLabel": "Diagnostico",
    "health.needsSetup": "requieren setup",
    "health.ready": "listas",
    "health.refresh": "Refrescar",
    "health.refreshTooltip": "Volver a ejecutar las comprobaciones locales.",
    "health.state.info": "info",
    "health.state.offline": "offline",
    "health.state.ready": "lista",
    "health.state.warning": "revisar",
    "health.title": "Preparacion del sistema",
    "model.fallback": "LM Studio local",
    "model.label": "Modelo",
    "permissions.blocked": "bloqueadas",
    "permissions.blockedSkills": "Habilidades bloqueadas",
    "permissions.copy": "Las habilidades sensibles estan bloqueadas por defecto. Para desbloquearlas, inicia Eve con",
    "permissions.loading": "Cargando perfil local de permisos.",
    "permissions.ready": "listas",
    "permissions.title": "Permisos",
    "quickActions.aria": "Acciones rapidas",
    "research.copy": "Busca, lee, prioriza y responde con fuentes citadas.",
    "research.title": "Investigacion web",
    "reset.label": "Nueva sesion",
    "reset.tooltip": "Borrar la sesion actual de Eve y empezar de nuevo.",
    "skills.catalogMissing": "Ejecuta `npm run r:catalog` si el catalogo no aparece.",
    "skills.fallbackDescription": "Habilidad R local.",
    "skills.loading": "cargando",
    "skills.searchAria": "Buscar herramientas R",
    "skills.searchPlaceholder": "Buscar PDF, CSV, QR...",
    "skills.title": "Habilidades R",
    "skills.use": "Usar",
    "skills.useTooltip": "Crear un prompt guiado para esta herramienta R concreta.",
    "status.backend": "Backend de Eve mediante proxy local",
    "status.error": "Error",
    "status.ready": "Lista",
    "status.responding": "Respondiendo",
    "status.sending": "Enviando",
    "system.blockedByDefault": "bloqueadas por defecto",
    "system.skillsIndexed": "habilidades indexadas",
    "system.toolsAvailable": "herramientas disponibles",
    "tools.label": "Herramientas",
    "tools.value": "Habilidades R / investigacion web / Firecrawl",
    "toolLog.empty": "Sin ejecuciones todavia",
    "toolLog.finished": "terminadas",
    "toolLog.history": "Historial",
    "toolLog.muted": "La actividad de herramientas aparecera aqui cuando actue el agente.",
    "toolLog.running": "en curso",
    "toolLog.runsThisSession": "ejecuciones en esta sesion",
    "toolLog.tools": "herramientas",
    "user.you": "Tu",
    "workbench.pdf.copy": "OCR, resumen, union, extraccion, reparacion y generacion de informes en local.",
    "workbench.pdf.title": "Mesa de trabajo PDF",
  },
};

export function t(lang: Lang, key: string): string {
  return translations[lang][key] ?? translations.en[key] ?? key;
}
